...
export default function AvatarConfigurationPage({ params }: { 
  ...
  const fetchSources = useCallback(async () => {
    ...
    try {
      ...
      const filesData = (await filesRes.json()).results || [];
  ...
  const saveSourcesAndTrain = async () => {
    ...
    try {
      const sourcesPayload = [
        ...
        ...(selectedFiles.length > 0 ? [{ source_type: "files", metadata: { file_ids: selectedFiles } }] : []),
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                  ...
                  <CollapsibleSourceSection title="Files" icon={FileText} items={files} selectedIds={selectedFiles} onToggle={(id) => setSelectedFiles(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])} getTitle={(f) => f.original_filename} getContent={(f) => f.content || "No preview available."} />
...
function setFiles(filesData: any) {
