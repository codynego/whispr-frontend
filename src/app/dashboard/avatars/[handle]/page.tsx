...
import {
  Settings, BarChart3, Loader2, AlertTriangle, Database, Trash2,
  Play, RefreshCw, CheckCircle2, Clock, FileText, Bell, CheckSquare,
  ChevronDown, Plus, File, Image, Paperclip
} from "lucide-react";
...
export default function AvatarConfigurationPage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  const fetchAvatar = useCallback(async (skipJobSync = false) => {
    ...
    try {
      ...
      if (!skipJobSync && data.last_training_job_id && !data.trained) {
  ...
  const fetchSources = useCallback(async () => {
    ...
    try {
      const [notesRes, remindersRes, todosRes, filesRes] = await Promise.all([
        ...
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/whisone/files/`, { headers: { Authorization: `Bearer ${accessToken}` } }),
      ]);
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <label key={item.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-white transition cursor-pointer">
                  ...
                  />
                  {title === "Files & Documents" && item.file_type === "image" ? (
                    <img src={item.file} alt="" className="w-10 h-10 rounded-lg object-cover border" />
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-20 h-20 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-2xl flex items-center justify-center text-white text-3xl font-bold">
                {avatar.name[0]}
              </div>
              <div>
                <h1 className="text-3xl font-bold text-gray-900">{avatar.name}</h1>
                <p className="text-lg text-gray-600">@{avatar.handle}</p>
              </div>
            </div>
            <div className="flex gap-2">
              {["training", "settings", "analytics"].map((tab) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab as Tab)}
                  className={`px-6 py-3 rounded-xl font-medium transition ${activeTab === tab
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="border-2 border-dashed border-emerald-300 rounded-2xl p-6 bg-emerald-50/30">
                    ...
                    <textarea
                      ...
                      placeholder="Paste bio, instructions, articles, or any text..."
              ...
              <div className="bg-white rounded-2xl shadow-lg p-6">
                ...
                {isTrainingActive ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-xs text-gray-500">Last trained: {avatar.trained_at ? new Date(avatar.trained_at).toLocaleDateString() : "Never"}</p>
                  </div>
                ) : (
                  <div className="text-center py-8">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="p-6 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl border border-emerald-200">
                <p className="text-sm text-emerald-700 font-medium mb-1">Total Messages</p>
